...
//    todo: check the result
    ...
    @ResponseBody
    public FileSystemResource getFileUsers( ) throws DocumentException {
        Document document = new Document();
        document.open();
        List <User> userList=bookingFacade.getAllUsers();
...
//    todo: error during closing document
    ...
    public FileSystemResource generateTicketsUserById(@RequestParam("id") long id) throws DocumentException, IOException {
        ...
        List <Ticket> ticketList=bookingFacade.getBookedTickets(user, 1,1);
        Paragraph para = new Paragraph();
